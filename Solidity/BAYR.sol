...
contract BAYR is IHALO {
    ...
    function allowance(address owner, address spender) public virtual override view returns (uint) {
        bool success;
        bytes memory result;
        (success, result) = proxy.staticcall(abi.encodeWithSignature("allowedReserve(address,address)",owner,spender));
    ...
    function increaseAllowance(address spender, uint value) public virtual override returns (bool) {
        ...
        bool success;
        bytes memory result;
        (success, result) = proxy.call(abi.encodeWithSignature("increaseAllowanceReserve(address,uint256,address)",spender,value,msg.sender));
        require(success);
    ...
    function decreaseAllowance(address spender, uint value) public virtual override returns (bool) {
        ...
        bool success;
        bytes memory result;
        (success, result) = proxy.call(abi.encodeWithSignature("decreaseAllowanceReserve(address,uint256,address)",spender,value,msg.sender));
        require(success);
        emit Approval(msg.sender, spender, allowance(msg.sender, spender));
        return true;
    }
