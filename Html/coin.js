...
(function () {
	...
	coinjs.cachedInputs = [];	//used for keeping cached data for wallet inputs
	coinjs.mTransactionProgress = {'start': 0, 'stop': 100, 'current': 0, 'progress': 0, 'step': 0};
...
https://api.latoken.com/v2/ticker //get all ticker data, unfortunately since BAY have 4 market pairs it is better to retrieve all of them at once instead of one by one

https://api.latoken.com/v2/ticker/BAY/BTC/
https://api.latoken.com/v2/ticker/BAYR/BTC
https://api.latoken.com/v2/ticker/BAYR/USDT
https://api.latoken.com/v2/ticker/BAY/USDT
...
*/
  ...
  /* retrieve coin price and data */
	coinjs.coinData = function(callback, provider, price_feed = "all"){
		
		if(!provider)
			provider = 'ratedb';

		coinjs.ajax(coinjs.coin_data[provider], callback, "GET");
		
		/*
		if (price_feed == "all")
			coinjs.ajax(coinjs.coin_data[provider], callback, "GET");
    else if(price_feed == "bay_btc")
      coinjs.ajax("https://api.latoken.com/v2/ticker/BAY/BTC", callback, "GET");
    else if(price_feed == "bay_usdt")
      coinjs.ajax("https://api.latoken.com/v2/ticker/BAY/USDT", callback, "GET");
        else if(price_feed == "bay_btc")
      coinjs.ajax("https://api.latoken.com/v2/ticker/BAYR/BTC", callback, "GET");
    else if(price_feed == "bay_usdt")
      coinjs.ajax("https://api.latoken.com/v2/ticker/BAYR/USDT", callback, "GET");
    else{}
    	*/
	...
	coinjs.pegInfo = function(callback){
		
			coinjs.ajax(coinjs.host + '/peginfo', callback, "GET");
  }
	/* retreive the balance from a given address */
	coinjs.addressBalance = function(address, callback){
      ...
      /*
      coinjs.ajax(coinjs.host + '/balance2/'+address, callback, "GET").then((quote) => {
      ...
      */
				...
				$.ajax ({
					...
					url: coinjs.host + '/balance/'+address,
					dataType: "json",
	...
	coinjs.transaction = function() {
		...
		r.listUnspent = function(address, callback) {
			...
			//coinjs.ajax('http://195.181.242.206:9998/api?method=blockchain.address.listunspent&params='+address+'&r='+Math.random(), callback, "GET");
				...
				//coinjs.createCORSRequest("POST", coinjs.host + '/listunspent/'+address)
		}
		...
		r.broadcast = function(callback, txhex){
			...
			//coinjs.ajax('http://195.181.242.206:9998/api?method=blockchain.transaction.broadcast&params='+tx+'&r='+Math.random(), callback, "GET");
				coinjs.ajax(coinjs.host + '/broadcast', callback, "POST", tx);
		...
		/* sign inputs */
		r.sign = async function(wif, sigHashType){
			
			var shType = sigHashType || 1;

			if (debug) {
				console.log("PrivKey signing started! sigHash type: " + shType);
			}
			...
			for (var i = 0; i < this.ins.length; i++) {
				...
				//console.log('sign_progressbar callback: ', callback);
					//await callback(i+1);
					//await sign_progressbar.go(coinjs.mTransactionProgress['step'] * i);
					
					await sign_progressbar.go(coinjs.mTransactionProgress['current'] + (coinjs.mTransactionProgress['step']));
					coinjs.mTransactionProgress['current'] = (coinjs.mTransactionProgress['current'] + coinjs.mTransactionProgress['step']);
					


				//for animation of the  progress bar
				await animateProgress(i+1);
		...
		/*sleep /pausefor animation to be kicked in the render queue */
		async function animateProgress(i){
			var ms = 20+i;
			if (i < 100)
				ms+= i;
			
			if(i % 10 == 0)	//every 10 txinput increase timeOut for antimation purpose
				ms+=20;

			//document.querySelector('.broadcast-process-bar').dataset.bar_total_txinputs_signed =  i; 
			document.querySelectorAll('.broadcast-process-bar')[0].dataset.bar_total_txinputs_signed =  i; 
			document.querySelectorAll('.broadcast-process-bar')[1].dataset.bar_total_txinputs_signed =  i; 
			//$('.broadcast-process-bar').data('bar_total_txinputs_signed', i)
			$(document).trigger('broadcast_progress_bar_changed');
	...
	//u = url, f = callback, m = method, a = parameters
	coinjs.ajax = function(u, f, m, a){
		...
		x.onreadystatechange=function(){
          ...
          console.log('coinjs.ajax error, url: ', u)
          PNotify_helper("Exception", "Couln't Fetch data from " + u, 'error');
        }
		};
